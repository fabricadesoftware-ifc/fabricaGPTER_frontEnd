<template>
  <v-card
    :class="['mx-0', { 'pa-4 mx-auto': smAndDown, 'pa-6': mdAndUp }]"
    max-width="550"
    :min-width="mdAndUp ? '340' : '90vw'"
  ...
  >
    <v-card-title
      ...
      :class="{ 'text-h6 mb-3': smAndDown, 'text-h5 mb-6': mdAndUp }"
    >
        ...
        <div v-for="info in infos" :key="info.prop" class="d-flex align-start ga-3">
          ...
          <div>
            ...
            <span
              :class="
                info.name === 'Status'
                  ? 'ml-1 text-subtitle-1 bg-blue-lighten-4 text-blue-darken-4 rounded-xl px-2 py-1'
                  : 'ml-1 text-subtitle-1'
              "
              >{{ info.prop }}</span
            >
          </div>
